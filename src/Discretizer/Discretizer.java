		...
		try {
			...
			bHit = false;
                    for (Instance instance : instances) {
                        boolean[] missing = instance.getInputMissingValues();
                        String newInstance="";
                        for (int j = 0; j<Parameters.numAttributes; j++) {
                            if (j == iClassIndex) {
                                String className = instance.getOutputNominalValues(0);
                            ...
                            if (missing[j]) {
                                ...
                                if (realAttributes[j]) {
                                    double val = instance.getInputRealValues(j);
                                    int interv=discretize(j,val);
                                    newInstance+=interv;
                                } else {
                                    newInstance += instance.getInputNominalValues(j);
                                }
                            }
